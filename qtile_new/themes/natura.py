"""Natura — the muted forest-green Cozytile palette, with the tuned bar.

The darkest theme of the set: its two surfaces are only ~17 levels apart, so the
gradient's lower stop is held clear of the bar background (see the generator's
entry for this theme).
"""
...
palette = {
    "outer": "#0F1212",  # bar background, the gaps between islands
    "island": "#202222",  # raised widget groups
    "fg": "#607767",
    "accent": "#B2BEBC",  # current workspace
    # Upstream leaves this at Cozy's purple rather than a Natura tone.  It is
    # inert under highlight_method "block", so it is kept as shipped.
    "glow": "#4B427E",
    "border": "#3b4252",
    "float": "#1F1D2E",
}
...
#   python3 ~/.config/qtile/scripts/gen-gradient-slices.py natura
GRADIENT = ("#303232", "#141717")
