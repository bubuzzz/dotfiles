...
keys = [
    ...
    # A floating window has no layout to fall back on, so "make this fill the
    # screen" needs its own key.  qtile's own maximize is a real float state,
    # not a resize: it stashes the window's geometry on the way in and puts it
    # back on the way out, so the second press returns the window to wherever
    # you had dragged it.  On a *tiled* window it floats-and-fills, and the
    # second press drops it back into the tiling layout.
    #
    # It fills screen.dwidth/dheight -- the usable area, so the bar and the
    # theme's screen_gaps are respected, but not the per-window layout margin.
    # A maximized float therefore sits GAP/2 closer to the screen edge than a
    # tiled Max window does.
    Key(
        [mod],
        "m",
        lazy.window.toggle_maximize(),
        desc="Maximize the focused window, or restore it",
    ),
    Key(
        [mod],
        "t",
